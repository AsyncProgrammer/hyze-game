...
class ZarosAltar : DialoguePlugin(){
    ...
    override fun build(player: Player, npcId: Int) = player.createDialogue {
        option("Trocar os cursos de oração?"){
            option("Sim, troque meu curso de oração"){
                if(!player.prayer.isAncientCurses){
                    plain("O altar enche sua cabeça com pensamentos sombiros, limpando as orações da sua memória e deixando apenas maldições em seu lugar.")
                    player.prayer.setPrayerBook(true)
                }else{
                    plain("O altar facilita o seu aperto no seu meio. As maldições escapam de sua memória e você se lembra das orações que costumava conhecer")
